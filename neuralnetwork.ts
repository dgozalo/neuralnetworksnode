...
const show = (varName: any) : string => {
    varName = Array.isArray(varName) ? varName : [varName]
    const transf = varName.map((v:string) => `${v}: ${parser.get(v)}`);
    return transf.join(', ');
}

const calc = (varName: string, expr: string, expanded?: string) => {
...
const calculoNeuronas = (iteracion: number) => {
